...
UnitIDRegistrationForm.propTypes = {
...
export default function UnitIDRegistrationForm({ isEdit, currentData, onCancel, onLoadData }) {
  ...
  const handleRegister = async () => {
    ...
    if (!isEdit) {
      try {
        ...
        const response = await createUpdateData(`${BASE_URL}/create`, 'unitID', createParams);
        if (response.httpStatusCode === 200) {
          onProcessSuccess();
          DthMessage({ variant: 'success', message: translate(`message.register_unit_ID_successful`) });
        }
      } catch (error) {
      ...
      try {
        ...
        const response = await createUpdateData(`${BASE_URL}/update`, 'unitID', updateParams);
        if (response.httpStatusCode === 200) {
          onProcessSuccess();
          DthMessage({ variant: 'success', message: translate(`message.update_unitID_successful`) });
        }
      } catch (error) {
        onProcessError(error);
      }
  ...
  const UnitIDSchema = Yup.object().shape({
    ...
    sum: Yup.string().required('Sum is required'),
    fp: Yup.string().required('FP is required')
  });
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      sum: (isEdit && currentData?.sum) || '',
      fp: (isEdit && currentData?.fp) || ''
    },
    validationSchema: UnitIDSchema,
              ...
              <Stack spacing={3}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  ...
                  <Dropdown
                    ...
                    defaultValue
                    errorMessage={touched.fp && errors.fp}
                  />
                </Stack>
              </Stack>
            </Card>
