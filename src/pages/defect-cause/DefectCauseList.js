...
import CauseClassRegisterForm from './CauseClassRegisterForm';
import CauseDetailRegisterForm from './CauseDetailRegisterForm';
// --------------------------------------------------

const pageCode = 'menu.masterData.production.qualityMasterData.defectItem.defectCause';
const tableCodeClass = 'defectCauseClass';
const tableCodeDetail = 'defectCauseDetail';
...
export default function DefectCauseList() {
  ...
  // }, [selectedWidget]);
  useEffect(() => {
    if (!isEmpty(currentSymptomClass)) {
  ...
  const onLoadData = (table = tableCodeClass) => {
    if (table === "defectCauseClass") {
      const _params = {
        classYn: searchParams.class.stateClass,
        sympClsCode: searchParams.class.causeClsCode,
        sympClsName: searchParams.class.causeClsName,
      }
    ...
    else if (table === 'defectCauseDetail') {
      const _searchParams = {
        ...searchParams,
        detail: {
          ...searchParams.detail,
      ...
      const _params = {
        ...
        sympName: searchParams.detail.causeName,
        sympCode: searchParams.detail.causeCode,
        sympClsCode: currentSymptomClass.code,
  ...
  const onInquiry = () => {
    onLoadData("defectCauseClass");
    if (!isEmpty(currentSymptomClass)) onLoadData('defectCauseDetail')
  };
      ...
      detail: (
        <>
          ...
          <TextField
            ...
            value={searchParams.class.causeClsCode}
          ...
          <TextField
            ...
            value={searchParams.class.causeClsName}
            onChange={handleChangeSearchClassConfig}
            sx={{ my: 1 }}
            size="small"
          />
          <TextField
            ...
            name="causeCode"
            label="Cause Code"
            value={searchParams.detail.causeCode}
          ...
          <TextField
            ...
            name="causeName"
            label="Cause Name"
            value={searchParams.detail.causeName}
            onChange={handleChangeSearchDetailConfig}
            sx={{ my: 1 }}
            size="small"
          />
  ...
  const onSelectionChangedDetail = (event) => {
    ...
    if (rowCount === 1) {
      ...
      if (selectedSymptomDetailID) {
        query({
          url: `/v1/cause/detail/${selectedSymptomDetailID}`,
          featureCode: 'user.create'
        })
          .then((res) => {
            const { data } = res;
  ...
  const onSelectionChangedClass = (event) => {
    ...
    if (rowCount === 1) {
      ...
      if (selectedSymptomClassID) {
        query({
          url: `/v1/cause/${selectedSymptomClassID}`,
          featureCode: 'user.create'
        })
          .then((res) => {
            const { data } = res;
  ...
  const handleDeleteDetail = async () => {
    ...
    await mutate({
      ...
      if (res.httpStatusCode === 200) {
        setSubmitting(false);
        enqueueSnackbar(translate(`message.defect_cause_detail_was_deleted_successfully`), {
          variant: 'success',
          action: (key) => (
            <MIconButton size="small" onClick={() => closeSnackbar(key)}>
              <Icon icon={closeFill} />
  ...
  const handleDeleteClass = async () => {
    ...
    await mutate({
      ...
      if (res.httpStatusCode === 200) {
        setSubmitting(false);
        enqueueSnackbar(translate(`message.defect_cause_was_deleted_successfully`), {
          variant: 'success',
          action: (key) => (
            <MIconButton size="small" onClick={() => closeSnackbar(key)}>
              <Icon icon={closeFill} />
      ...
      <Container sx={{ px: `0px !important` }} maxWidth={false}>
        <Grid container spacing={0} sx={{ px: 0, height: `calc(100vh - 254px)` }}>
          ...
          <Grid item xs={12} md={hideFilters ? 12 : 10}>
            ...
            >
              ...
              >
                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
                  <Typography variant="h5">{translate(`typo.defect_cause_class`)}</Typography>
                  <Stack
                    direction="row"
              ...
              >
                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
                  <Typography variant="h5">{translate(`typo.defect_cause_detail`)}</Typography>
                  <Stack
                    direction="row"
                    justifyContent="right"
                    display="flex"
            ...
            <DialogDragable
              title={`${isEdit ? translate(`typo.modify`) : translate(`typo.register`)} ${translate(`typo.defect_cause_class`)}`}
              maxWidth="lg"
              open={isOpenClassActionModal}
              onClose={handleCloseClassModal}
            >
              <CauseClassRegisterForm
                isEdit={isEdit}
            ...
            <DialogDragable
              title={`${isEdit ? translate(`typo.modify`) : translate(`typo.register`)} ${translate(`typo.defect_cause_detail`)}`}
              maxWidth="lg"
              open={isOpenDetailActionModal}
              onClose={handleCloseDetailModal}
            >
              <CauseDetailRegisterForm
                isEdit={isEdit}
