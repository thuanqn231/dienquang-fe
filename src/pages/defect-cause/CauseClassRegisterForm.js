...
CauseClassRegisterForm.propTypes = {
...
export default function CauseClassRegisterForm({
  isEdit,
  currentData,
  onCancel,
  onLoadData,
          ...
          .then((res) => {
            if (res.httpStatusCode === 200) {
              if (res.data !== []) {
                ...
                setIsOpenConfirmModal(false);
                onCancel();
                enqueueSnackbar(translate(`message.defect_cause_class_was_registered_successfully`), {
                  variant: 'success',
                  action: (key) => (
                ...
                setIsOpenConfirmModal(false);
                enqueueSnackbar(translate(`message.defect_cause_class_was_registered_unsuccessfully`), {
                  variant: 'warning',
                  action: (key) => (
                    <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                      <Icon icon={closeFill} />
        ...
        mutate({
          ...
          .then((res) => {
            if (res.httpStatusCode === 200) {
              ...
              setIsOpenConfirmModal(false);
              onCancel();
              enqueueSnackbar(translate(`message.defect_cause_class_was_updated_successfully`), {
                variant: 'success',
                action: (key) => (
                  <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                    <Icon icon={closeFill} />
