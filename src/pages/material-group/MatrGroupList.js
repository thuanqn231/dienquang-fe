...
import MatrGroupRegistrationForm from './MatrGroupRegistrationForm';

// ----------------------------------------------------------------------

export default function MatrGroupList() {
  const { translate } = useLocales();
  const [isOpenActionModal, setOpenActionModal] = useState(false);
  const [currentMatrGroup, setCurrentMatrGroup] = useState({});
  const [isEdit, setIsEdit] = useState(false);
  const [selectedMatrGroupId, setSelectedMatrGroupId] = useState(null);
  ...
  useEffect(async () => {
    if (selectedMatrGroupId) {
      const data = await loadSelectedData(BASE_URL, selectedMatrGroupId);
      setCurrentMatrGroup({
  ...
  const onRowSelected = (selectedNodes) => {
    ...
    if (rowCount === 0) {
      setSelectedMatrGroupId(null);
    }
    if (rowCount === 1) {
      const matrGroupId = selectedNodes[0].data.factoryPk;
      setSelectedMatrGroupId(matrGroupId);
    }
  };
  ...
  const selectWarning = () => {
    DthMessage({ variant: 'warning', message: translate(`message.please_select_1_material_group`) });
  }
  ...
  const handleDeleteMatrGroup = async () => {
    setSubmitting(true);
    try {
      const res = await deleteSelectedData(BASE_URL, selectedMatrGroupId);
      if (res.httpStatusCode === 200) {
        setSubmitting(false);
        DthMessage({ variant: 'success', message: translate(`message.material_group_delete_successful`) });
  ...
  return (
    <>
      ...
      />
      <DialogDragable
        title={`Material Group ${isEdit ? translate(`typo.modify`) : translate(`typo.registration`)}`}
        maxWidth="lg"
        open={isOpenActionModal}
        onClose={handleCloseModal}
      >
        <MatrGroupRegistrationForm
          isEdit={isEdit}
          currentData={currentMatrGroup}
      ...
      <DialogDragable title={translate(`typo.delete`)} maxWidth="sm" open={isOpenDeleteModal} onClose={handleCloseDeleteModal}>
        ...
        <DialogActions>
          ...
          <LoadingButton type="button" variant="contained" onClick={handleDeleteMatrGroup} loading={isSubmitting}>
