...
MatrGroupRegistrationForm.propTypes = {
...
export default function MatrGroupRegistrationForm({ isEdit, currentData, onCancel, onLoadData }) {
  ...
  const handleRegister = async () => {
    ...
    if (!isEdit) {
      try {
        const createParams = {
          code: values?.materialgroupCode.toUpperCase() || null,
          name: values?.materialgroupName || null,
        ...
        const response = await createUpdateData(`${BASE_URL}/create`, 'materialGroup', createParams);
        if (response.httpStatusCode === 200) {
          onProcessSuccess();
          DthMessage({ variant: 'success', message: translate(`message.register_material_group_successful`) });
        }
      } catch (error) {
      ...
      try {
        ...
        const response = await createUpdateData(`${BASE_URL}/update`, 'materialGroup', updateParams);
        if (response.httpStatusCode === 200) {
          onProcessSuccess();
          DthMessage({ variant: 'success', message: translate(`message.update_material_group_successful`) });
        }
      } catch (error) {
        onProcessError(error);
      }
  ...
  const MaterialGroupSchema = Yup.object().shape({
    factory: Yup.string().required('Factory is required'),
    state: Yup.string().required('Use (Y/N) is required'),
    materialgroupCode: Yup.string().required('Material Group Code is required'),
    materialgroupName: Yup.string().required('Material Group Name is required'),
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      materialgroupCode: isEdit && currentData?.materialgroupCode || '',
      materialgroupName: isEdit && currentData?.materialgroupName || '',
      description: isEdit && currentData?.description || ''
    },
    validationSchema: MaterialGroupSchema,
              ...
              <Stack spacing={3}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  <TextField
                    ...
                    required
                    disabled={isEdit}
                    {...getFieldProps('materialgroupCode')}
                    error={Boolean(touched.materialgroupCode && errors.materialgroupCode)}
                    helperText={touched.materialgroupCode && errors.materialgroupCode}
                  />
                  <TextField
                    autoComplete="off"
                    fullWidth
                    ...
                    required
                    {...getFieldProps('materialgroupName')}
                    error={Boolean(touched.materialgroupName && errors.materialgroupName)}
                    helperText={touched.materialgroupName && errors.materialgroupName}
                  />
                </Stack>
                <TextField
                  autoComplete="off"
