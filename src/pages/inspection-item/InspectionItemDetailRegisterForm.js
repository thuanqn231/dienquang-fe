...
InspectionItemDetailRegisterForm.propTypes = {
...
export default function InspectionItemDetailRegisterForm({
  isEdit,
  currentData,
  onCancel,
  onLoadData,
  ...
  const handleRegister = () => {
    ...
    if (!isEdit) {
      try {
        mutate({
          url: '/v1/inspection-item/detail/create',
          data: {
          ...
          .then((res) => {
            if (res.httpStatusCode === 200) {
              if (res.data !== []) {
                ...
                setIsOpenConfirmModal(false);
                onCancel();
                enqueueSnackbar(translate(`message.inspect_item_detail_was_registered_successfully`), {
                  variant: 'success',
                  action: (key) => (
                ...
                setIsOpenConfirmModal(false);
                enqueueSnackbar(translate(`message.inspect_item_detail_was_registered_unsuccessfully`), {
                  variant: 'warning',
                  action: (key) => (
                    <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                      <Icon icon={closeFill} />
      ...
      try {
        mutate({
          url: '/v1/inspection-item/detail/update',
          data: {
            inspectionItemDetailUpdate: {
          ...
          .then((res) => {
            if (res.httpStatusCode === 200) {
              ...
              setIsOpenConfirmModal(false);
              onCancel();
              enqueueSnackbar(translate(`message.inspect_item_detail_was_updated_successfully`), {
                variant: 'success',
                action: (key) => (
                  <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                    <Icon icon={closeFill} />
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      classType: currentClass?.inspectionType?.code
    },
