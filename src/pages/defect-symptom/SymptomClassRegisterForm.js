...
SymptomClassRegisterForm.propTypes = {
...
export default function SymptomClassRegisterForm({
  isEdit,
  currentData,
  onCancel,
  onLoadData,
  ...
  const handleRegister = () => {
    ...
    if (!isEdit) {
      try {
        mutate({
          url: '/v1/symptom/create',
          data: {
            code: valuesForm?.code,
            name: valuesForm?.name,
            sortOrder: Number(valuesForm?.sortOrder),
          ...
          .then((res) => {
            if (res.httpStatusCode === 200) {
              if (res.data !== []) {
                ...
                setIsOpenConfirmModal(false);
                onCancel();
                enqueueSnackbar(translate(`message.defect_symptom_class_was_registered_successfully`), {
                  variant: 'success',
                  action: (key) => (
                ...
                setIsOpenConfirmModal(false);
                enqueueSnackbar(translate(`message.defect_symptom_class_was_registered_unsuccessfully`), {
                  variant: 'warning',
                  action: (key) => (
                    <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                      <Icon icon={closeFill} />
        ...
        mutate({
          ...
          .then((res) => {
            if (res.httpStatusCode === 200) {
              ...
              setIsOpenConfirmModal(false);
              onCancel();
              enqueueSnackbar(translate(`message.defect_symptom_class_was_updated_successfully`), {
                variant: 'success',
                action: (key) => (
                  <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                    <Icon icon={closeFill} />
  ...
  const symptomClassRegSchema = Yup.object().shape({
    ...
    code: Yup.string().required('Code is required').test('len', 'Must be exactly 2 characters', val => {
      if (val === undefined) {
        return true;
      }
      return val.length === 2
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      sortOrder: (isEdit && currentData?.sortOrder) || '',
    },
