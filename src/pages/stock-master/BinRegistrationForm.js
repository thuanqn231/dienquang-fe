...
export default function BinRegistrationForm({ isEdit, currentData, onCancel, onLoadData }) {
  ...
  const handleRegister = () => {
    ...
    const _binRow = `0${valuesForm?.binRow}`.slice(-2);
    const _binColumn = `0${valuesForm?.binColumn}`.slice(-2);
    if (!isEdit) {
      try {
        mutate({
          url: '/v1/stock-management/bin/create',
          data: {
            bin: {
              code: `${_binRow}${_binColumn}`,
              name: `${_binRow}${_binColumn}`,
              caPaLimit: valuesForm?.capaLimit || null,
              state: valuesForm?.state || 'RUNNING',
              zone: {
                factoryPk: valuesForm?.zone || ''
              },
              pk: {
          ...
          if (res.httpStatusCode === 200) {
            ...
            enqueueSnackbar(translate(`message.register_bin_successful`), {
              variant: 'success',
              action: (key) => (
      ...
      try {
        mutate({
          url: '/v1/stock-management/bin/update',
          data: {
            bin: {
          ...
          if (res.httpStatusCode === 200) {
            ...
            onCancel();
            updateCommonDropdown();
            enqueueSnackbar(translate(`message.update_bin_successful`), {
              variant: 'success',
              action: (key) => (
                <MIconButton size="small" onClick={() => closeSnackbar(key)}>
                  <Icon icon={closeFill} />
  ...
  const UOMSchema = Yup.object().shape({
    ...
    binRow: Yup.string().length(2, 'Bin Row must be 2 characters').matches(/^[0-9]+$/, "Allow number only").required('Bin Row is required'),
    binColumn: Yup.string().length(2, 'Bin Column must be 2 characters').matches(/^[0-9]+$/, "Allow number only").required('Bin Column is required'),
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      zone: isEdit && currentData?.zone || '',
      binRow: isEdit && currentData?.binRow || '',
      binColumn: isEdit && currentData?.binColumn || '',
              ...
              <Stack spacing={3}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  ...
                  <Dropdown
                    ...
                    required
                    onChange={handleChange}
                    options={commonDropdown.zoneDropdown.filter((zone) => zone.factory === values.factory && zone.stock === values.stock)}
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  <TextField
                    ...
                    required
                    disabled={isEdit}
                    {...getFieldProps('binRow')}
                    error={Boolean(touched.binRow && errors.binRow)}
                    helperText={touched.binRow && errors.binRow}
                  />
                  <TextField
                    autoComplete="off"
                    fullWidth
                    ...
                    required
                    disabled={isEdit}
                    {...getFieldProps('binColumn')}
                    error={Boolean(touched.binColumn && errors.binColumn)}
                    helperText={touched.binColumn && errors.binColumn}
                  />
                </Stack>
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }}>
                  <TextField
