...
function NavItem({ item, active, isShow }) {
  ...
  const isActiveRoot = active(item.path, 'root');
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <List component="div" disablePadding>
              {children.map((item) => {
                ...
                const isActiveSub = active(path, 'sub');
...
export default function NavSection({ navConfig, isShow = true, ...other }) {
  ...
  const match = (path, level) => {
    if (path) {
      if (!Number.isNaN(path.slice(-6)) && level === 'root') {
        path = `${path.slice(0, -3)}000`;
        const newPathName = `${pathname.slice(0, -3)}000`;
        return !!matchPath({ path, end: false }, newPathName)
      }
  ...
  return (
    <Box {...other}>
      {navConfig.map((list, idx) => {
        const { subheader, children } = list;
        return (
          <List key={`${subheader}-${idx}`} disablePadding>
