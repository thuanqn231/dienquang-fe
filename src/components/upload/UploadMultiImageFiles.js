  ...
  return (
    <Box sx={{ width: '100%', ...sx }}>
      {!isDownload && (
        ...
        >
          <input {...getInputProps()} />
          <Box sx={{ p: 2, ml: { md: 2 }, textAlign: 'center' }}>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              {`(${translate(`typo.images_file_below_1_mb`)})`}
            </Typography>
          </Box>
        </DropZoneStyle>
      )}
            ...
            {files.map((file) => {
                ...
                return (
                  <ListItem
                    ...
                    sx={{
                      ...
                      display: 'inline-flex',
                      '&:hover': {
                        cursor: isEdit || (isView && 'pointer')
                      }
                    }}
                    onClick={isEdit || (isView && (() => onDownLoadFile(name, file.downloadUrl)))}
                  >
                    <Paper
                      variant="outlined"
                      component="img"
                      src={file.url || (isString(file) ? file : preview)}
                      sx={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute' }}
                    />
                    {!isView && (
                      <Box sx={{ top: 6, right: 6, position: 'absolute' }}>
                        <Button
                          sx={{ top: -3, right: -25, position: 'absolute' }}
                          size="small"
              ...
              return (
                ...
                >
                  ...
                  <ListItemText
                    primary={`${isString(file) ? file : name} (${isString(file) ? '' : fData(size)})`}
