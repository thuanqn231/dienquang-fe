...
export default function UploadAvatar({ error, file, caption, sx, ...other }) {
  ...
  if (file == null) _file = file;
    ...
    if (Array.isArray(file)) {
      if (typeof file[0] === "object") [_file, ...other] = file
      else _file = null;
    }
    else _file = file;
        ...
        >
          ...
          {!isEmpty(_file) && (
            <Box
              component="img"
              alt="avatar"
              src={_file.url !== undefined ? _file.url : _file.preview}
          ...
          >
            ...
            <Typography variant="caption">{_file ? translate(`typo.update_photo`) : translate(`typo.update_photo`)}</Typography>
          </PlaceholderStyle>
        </DropZoneStyle>
      </RootStyle>
